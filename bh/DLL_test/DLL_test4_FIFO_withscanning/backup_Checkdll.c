...
float collection_time = 6.0;

char data_dir_path[255] = "C:\\Users\\TCSPC\\Desktop\\";
char coords_fname[255] = "points.dat";
char coords_fpath[255];
char phot_basename[255] = "point_%03d.spc";
char sdt_basename[255] = "point_%03d.sdt";
char phot_fpath[255], phot_fpath_[255];
char sdt_fpath[255], sdt_fpath_[255];
short SPC_ACTIVE_MOD = 0;
float zoom_factor = 4.00;
int nx = 512;
int ny = 512;
...
int main()
{
    ...
    */
    ...
    /*
    setup for FIFO image measurement
    */
    ...
    SPC_configure_memory(spc_act_mod, spc_data.adc_resolution, 7, &spc_mem_info);

    SPC_set_page(spc_act_mod, meas_page);   // before fill

    ret = SPC_fill_memory(spc_act_mod, -1, meas_page, offset_value);
    if (ret > 0) {
        // fill started but not yet finished
        ret = test_fill_state();
    }
    if (ret < 0) {
        printf("L%d: Error %d during memory fill.\n", __LINE__, ret);
        return -1;
    }
    ...
    if (curr_mode != ROUT_OUT && curr_mode != FIFO_32M) {
        printf("setting now...\n");
        SPC_set_parameter(spc_act_mod, MODE, FIFO_32M);
        ...
        SPC_set_parameter(spc_act_mod, ZOOM_FACTOR, zoom_factor);   // image zoom
        SPC_set_parameter(spc_act_mod, SCAN_SIZE_X, nx);   // image size nx * ny pixels
        SPC_set_parameter(spc_act_mod, SCAN_SIZE_Y, ny);

        float fv;
        SPC_get_parameter(spc_act_mod, ZOOM_FACTOR, &fv);
        printf("spc zoom factor = %lf\n", fv);
        SPC_set_parameter(spc_act_mod, SCAN_ROUT_X, 7);    // without routing
        SPC_set_parameter(spc_act_mod, SCAN_ROUT_Y, 7);
        curr_mode = ROUT_OUT;
    }
    ...
    SPC_configure_memory(spc_act_mod, -1, 0, &spc_mem_info);
    ...
    for (short i = 0; i < MAX_NO_OF_SPC; i++) {
        if (mod_active[i]) {
            ...
            if (spc_state & SPC_ARMED) {  //  system armed
                if ((spc_state & SPC_MEASURE) == 0) {
                  ...
                  // if required , rates can be read
                    ...
                    //    }
                    //}
                    if (new_time - old_time > collection_time)
                    {
                        for (int i = 0; i < MAX_NO_OF_SPC; i++) {
                            if (!mod_active[i]) continue;
