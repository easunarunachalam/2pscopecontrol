...
// assumption is done here that we work with SPC-150 modules
...
if( !total_no_of_spc){
  ...
  //    2 - use SPC_set_mode function
            ...
            /* SPC150 simulation mode for 4 modules */
  for( i =0 ; i < MAX_NO_OF_SPC; i++)
    mod_active[i] = 1;
  force_use = 0;
  spc_error = SPC_set_mode( SPC_SIMUL150, force_use, mod_active);
  work_mode = SPC_get_mode();
  if( work_mode != SPC_SIMUL150)
    return -1;
...
if( module_type < M_SPC700 && module_type != M_SPC140){
  ...
  SPC_get_parameter( act_mod, COLLECT_TIME, &spc_dat.collect_time);
            // stop on time must be set for Continuous Flow measurement
...
//  part 3 -  example of Scan Sync In measurement     - not for SPC13x/6x0    
  
////////////////////////////////////////////////////////////////

if( module_type >= M_SPC700 || ( module_type >= M_SPC140 && module_type < M_DPC230 ){
  ...
  if ( module_type < M_SPC150 && module_type > M_SPC187 )
    SPC_enable_sequencer ( -1, 1 );    
