...
// run as follows:
// PS C:\Users\Arunachalam\Documents\GitHub\2pscopecontrol\bh\FCS_engine\linescan_FIFO\out\build\x64-debug>
...
short spc_state, armed, init_status, module_type, fifo_stopt_possible, first_write, spc_error, force_use, spc_act_mod, block_length, fifo_type, spc_ret, ret1;
...
float park_offset_x, park_offset_y, zoom_factor, fval;
...
char coords_fname[255] = "\\line_positions.dat";
...
int main(int argc, char** argv)
{
    ...
    if (argc != 5) {
        ...
        zoom_factor = std::atof(argv[4]);
        printf("Zoom factor set as: %2.2f\n", zoom_factor);
    }
    ...
    */
    ...
    if (coords_fptr == NULL) {
        ...
        do {
            ...
            gvd_ret = GVD_set_parameter(gvd_act_mod, SCAN_TYPE, 1);
            gvd_ret = GVD_set_parameter(gvd_act_mod, ZOOM_FACTOR, zoom_factor);
            gvd_ret = GVD_set_parameter(gvd_act_mod, OFFSET_X, scanner_target_x);
            gvd_ret = GVD_set_parameter(gvd_act_mod, OFFSET_Y, scanner_target_y);
            gvd_ret = GVD_prepare_scan_curve(gvd_act_mod);
            gvd_ret = GVD_get_scan_info(gvd_act_mod, &scan_info);
