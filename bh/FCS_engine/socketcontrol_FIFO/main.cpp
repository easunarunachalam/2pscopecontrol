...
short initialize_DCS_hardware(void);
...
int __cdecl main(void)
{
    ...
    // initialize hardware
    // status = initialize_DCS_hardware();
    ...
    do {
        ...
        if (iResult > 0) {
            ...
            // tokenize received text, stripping nullspace first
            std::string recvbuf_s = std::string(recvbuf);
            // recvbuf_s.erase(std::find(recvbuf_s.begin(), recvbuf_s.end(), '\0'), recvbuf_s.end());
            std::vector<std::string> tokens;
            boost::split(tokens, recvbuf_s, boost::is_any_of(", \0 \n"), boost::token_compress_on);
    ...
    // shutdown the connection and DCS hardware
